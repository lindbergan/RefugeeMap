 ...
 *
 * Taken from:
 * http://stackoverflow.com/questions/10913384/how-to-make-textview-wrap-its-multiline-content-exactly
 */
		...
		if (hasMaxWidth) {
			...
			if (specModeW != MeasureSpec.EXACTLY) {
				...
				if (linesCount > 1) {
					...
					{
						textRealMaxWidth = Math.max(textRealMaxWidth,
							layout.getLineWidth(n));
					}
					int w = Math.round(textRealMaxWidth);
					if (w < getMeasuredWidth()) {
						super.onMeasure(MeasureSpec.makeMeasureSpec(w,
