 ...
 * Also initiates graphic updates of relevant {@link Fragment}
 * when saving/removing an {@link Event}.
 */

public class SavedEventsHelper {

	private final Activity mActivity;
	private final List<SavedEventListener>
		savedEventListeners = new ArrayList<>();
	...
	public boolean onSaveEventButtonPressed(String id) {
		...
		Set<String> savedEvents = prefs.getStringSet(mActivity.getString(R.
			string.saved_events_key), new TreeSet<String>());
		Set<String> updatedEventList = new TreeSet<>(savedEvents);

		if (!savedEvents.contains(id)) {

			updatedEventList.add(id);
			editor.putStringSet(mActivity.getString(R.string.saved_events_key),
				updatedEventList);
	...
	public List<Event> getSavedEvents() {
		Set<String> savedEventsStr = mActivity.getPreferences(Context.
			MODE_PRIVATE).getStringSet(mActivity.getString(R.
			string.saved_events_key), null);
	...
	public void updateSavedEventsFrag(){
		...
		Fragment frag = mActivity.getFragmentManager().
			findFragmentByTag("saved_events_list_frag");
	...
	public interface SavedEventListener {
		void onSavedEvent(List<Event> savedEvents);
	}
	 ...
	 */
	private void updateSavedEventListeners() {
